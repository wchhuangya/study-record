...
没有这个属性
Traceback (most recent call last):
  File "/Users/wchya/project/vscode/python-start/circle.py", line 945, in <module>
    print(getattr(Test, 'b'))
...
print(getattr(Test, 'a'))
print(getattr(Test, 'b', '没有该属性'))
setattr(Test, 'b', '新增的属性')
print(getattr(Test, 'b', '没有该属性'))
...
如果参数包含方法 `__dir__()` ，该方法将被调用，如果不包含，该方法将最大限度的收集参数信息
...
2. Python 每个模块都有一个内置的 `__name__` 变量，当且仅当文件作为程序运行时，而不是作为库导入时，Python 会将这个变量设为 `__main__` 字符串。因此，如果在一个脚本文件的最后几行写下如下的代码，那么当这个叫本文件作为顶层程序运行时，文件最后几行会自动执行这个脚本，但在其他地方被导入时则不会执行函数：
   ...
   # 当这个文件作为顶层程序运行时，就会在控制台上打印出：test 的文本，但是如果它通过 import 等方式导入到其他程序中时，test 并不会打印
   ```

   这个技巧的提议意义：用函数的形式来编写程序，而不是作为顶层代码，这样可以方便以后的导入和重用。
